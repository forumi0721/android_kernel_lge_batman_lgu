...
#if defined(CONFIG_MACH_LGE_I_BOARD_VZW) || defined(CONFIG_MACH_LGE_325_BOARD)
...
void* LGF_TestScriptItemSet(test_mode_req_type * pReq, DIAG_TEST_MODE_F_rsp_type * pRsp)
{
    ...
    switch(pReq->test_mode_test_scr_mode)
    {
        case TEST_SCRIPT_ITEM_SET:
            ...
            #if defined (CONFIG_MACH_LGE_I_BOARD_LGU) || defined (CONFIG_MACH_LGE_I_BOARD_VZW) || defined (CONFIG_MACH_LGE_325_BOARD_VZW)|| defined(CONFIG_MACH_LGE_325_BOARD)
            send_to_arm9((void*)&req_ptr, (void*)pRsp);
...
PACK (void *)LGE_Dload_SRD (PACK (void *)req_pkt_ptr, uint16 pkg_len)
{
    ...
    switch(req_ptr->header.sub_cmd)
    {
        ...
        case USERDATA_BACKUP_REQUEST:
            remote_rpc_srd_cmmand(req_ptr, rsp_ptr);  //userDataBackUpStart() ??기??... shared ram ??????도??? .. 
        ...
        case USERDATA_BACKUP_REQUEST_MDM:
            ...
            remote_rpc_srd_cmmand(req_ptr, rsp_ptr); //userDataBackUpStart() ??기??... ram ??????도??? .. 
            diag_userDataBackUp_entrySet(req_ptr,rsp_ptr,1); //write info data ,  after rpc respons include write_sector_counter  remote_rpc_srd_cmmand(req_ptr, rsp_ptr);  //userDataBackUpStart() ??기??... ram ??????도??? .. 
            write_size= rsp_ptr->rsp_data.write_sector_counter *256; //return nv backup counters  
