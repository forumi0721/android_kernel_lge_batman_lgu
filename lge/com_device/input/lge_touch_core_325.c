...
int get_touch_ts_fw_version(char *fw_ver)
{
	if (touch_test_dev)
	{
		sprintf(fw_ver, "%d.%c%c%c%c", touch_test_dev->fw_info.manufacturer_id, touch_test_dev->fw_info.fw_config_id[0]
		, touch_test_dev->fw_info.fw_config_id[1], touch_test_dev->fw_info.fw_config_id[2], touch_test_dev->fw_info.fw_config_id[3]);
		return 1;
	}
	else
	{
...
static int touch_ic_init(struct lge_touch_data *ts)
{
	...
	if(ts->pdata->role->operation_mode == INTERRUPT_MODE){
		...
		if(unlikely((int_pin != 1 && next_work <= 0) || (int_pin == 1 && next_work >= 1))){
...
static int touch_ic_custom(struct lge_touch_data *ts)
{
	...
	if (touch_device_func->custom == NULL) {
		TOUCH_INFO_MSG("There is no specific custom function\n");
		goto err_out_critical;
	}

	if (touch_device_func->custom) {
		ret = touch_device_func->custom(ts->client);
		if (ret < 0) {
			TOUCH_ERR_MSG("custom family read fail\n");
			goto err_out_retry;
		}
	}

	if (unlikely(touch_debug_mask & (DEBUG_BASE_INFO | DEBUG_GHOST))){
		TOUCH_INFO_MSG("Customer Family [%d]\n",  customer_family);
	}
	...
	if(ts->gf_ctrl.stage & GHOST_STAGE_1){
		if(ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0){
			if(ts->gf_ctrl.count < ts->gf_ctrl.min_count || ts->gf_ctrl.count >= ts->gf_ctrl.max_count) {
				if(ts->gf_ctrl.ghost_check_count < MAX_GHOST_CHECK_COUNT-1)
				ts->gf_ctrl.ghost_check_count = 0;
				else
			...
			if(ts->gf_ctrl.ghost_check_count >= MAX_GHOST_CHECK_COUNT){
					iRet = touch_i2c_read(ts->client, PALM_PRESENT_REG, 1, (unsigned char *)&uPalm);
					if (iRet <0) {
						TOUCH_ERR_MSG("PALM PRESENT READ Fail\n");
						return iRet;
						}
							if (uPalm & 0x02) {
							iRet = 2;
							if (unlikely(touch_debug_mask & DEBUG_GHOST))
							TOUCH_DEBUG_MSG("Palm Present %x, %x\n", PALM_PRESENT_REG, iRet);
							}
		...
		if(ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0){
			...
			if(touch_device_func->ic_ctrl){
				if (touch_device_func->ic_ctrl(ts->client, IC_CTRL_BASELINE, PALM_SUPPRESSION_ON) < 0)
					return -1;
			}
		...
		if(ts->ts_data.total_num == 0 && ts->ts_data.curr_button.state == 0){
			...
			if (unlikely(touch_debug_mask & DEBUG_GHOST))
				TOUCH_INFO_MSG("ghost_stage_4: ghost_check_count+[0x%x]\n", ts->gf_ctrl.ghost_check_count);
			...
			if (unlikely(touch_debug_mask & DEBUG_GHOST))
				TOUCH_INFO_MSG("ghost_stage_4: multi_finger. return to ghost_stage_1[0x%x]\n", ts->gf_ctrl.stage);
		}
	}
	return 0;
}
...
static void touch_fw_upgrade_func(struct work_struct *work_fw_upgrade)
{
  ...
  TOUCH_INFO_MSG("FW Ver.[%c%c%c%c] \n", ts->fw_info.fw_config_id[0]
					, ts->fw_info.fw_config_id[1], ts->fw_info.fw_config_id[2], ts->fw_info.fw_config_id[3]);
#if defined (CONFIG_MACH_LGE_325_BOARD_DCM)
  if (lge_bd_rev < LGE_REV_B)
  {
    TOUCH_INFO_MSG("FW-upgrade is not executed DCM (Low revision)\n");
		goto out;
  }
#elif defined (CONFIG_MACH_LGE_325_BOARD_SKT) || defined (CONFIG_MACH_LGE_325_BOARD_LGU)
  if (lge_bd_rev < LGE_REV_D)
  {
    TOUCH_INFO_MSG("FW-upgrade is not executed SKT & LGU (Low revision)\n");
		goto out;
  }
#endif
  ...
  if (lge_bd_rev >= LGE_REV_E){
	if ( (ts->fw_info.fw_config_id[3] == 0x32)  &&
	...
	if ( (ts->fw_info.fw_config_id[3] == 0x39)  &&
	        (ts->fw_info.fw_config_id[2] == 0x30) &&
	        (ts->fw_info.fw_config_id[1] == 0x30) &&
	        (ts->fw_info.fw_config_id[0] == 0x53)) {
		TOUCH_INFO_MSG("FW-upgrade is not executed (FW ID OK)\n");
	...
	if ( (ts->fw_info.fw_config_id[3] == 0x39)  &&
	        ...
	        (ts->fw_info.fw_config_id[0] == 0x53) &&
	        (customer_family == 0)) {
		TOUCH_INFO_MSG("FW-upgrade is not executed Old (FW ID OK)\n");
		goto out;
	}

	if ( (ts->fw_info.fw_config_id[3] == 0x33)  &&
	        (ts->fw_info.fw_config_id[2] == 0x30) &&
	        (ts->fw_info.fw_config_id[1] == 0x31) &&
	        (ts->fw_info.fw_config_id[0] == 0x53)&&
	        (customer_family == 1)) {
		TOUCH_INFO_MSG("FW-upgrade is not executed New(FW ID OK)\n");
		goto out;
	}
#elif defined (CONFIG_MACH_LGE_325_BOARD_VZW) && defined(CONFIG_BATMAN_VZW_KERNEL_BOARD_REV)
	if (lge_bd_rev_batman_vzw >= LGE_REV_B){
	  if ( (ts->fw_info.fw_config_id[3] == 0x33)  &&
			  (ts->fw_info.fw_config_id[2] == 0x30) &&
			  (ts->fw_info.fw_config_id[1] == 0x31) &&
			  (ts->fw_info.fw_config_id[0] == 0x53)) {
...
static int touch_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (ts->pdata->caps->is_id_supported)
		input_set_abs_params(ts->input_dev, ABS_MT_TRACKING_ID, 0, ts->pdata->caps->max_id - 1, 0, 0);
	...
	ret = touch_ic_custom(ts);
	if (touch_power_cntl(ts, POWER_OFF) < 0)
		goto err_power_failed;
	if (touch_power_cntl(ts, POWER_ON) < 0)
		goto err_power_failed;
	msleep(ts->pdata->role->booting_delay);
