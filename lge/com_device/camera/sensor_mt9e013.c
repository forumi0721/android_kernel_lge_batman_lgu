...
/*=============================================================
	Switching Log(jungki.kim@lge.com, 2011-11-03)
==============================================================*/
...
// jungki.kim@lge.com  2011-10-03  Read from EEPROM and Write to Sensor
//#define EEPROM_CHECK
...
static int32_t mt9e013_sensor_setting(int update_type, int rt)
{
	...
	mt9e013_stop_stream();
	msleep(5);	// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag
	if (update_type == REG_INIT) {
		...
		if (rt == QTR_SIZE) {
			...
			// jungki.kim@lge.com  2012-01-05  Not in use due to advanced C-AF
			//mt9e013_set_default_focus(0);	// jungki.kim@lge.com  2011-11-03  release Auto Focus when switching mode except capture.

			// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag {
			//mt9e013_i2c_write_w_table(mt9e013_regs.reg_pll,	mt9e013_regs.reg_pll_size);
			//msleep(1);
			// sungsik.kim@lge.com  2011-11-03  To Reduce Shutter-lag }
			mt9e013_i2c_write_w_table(mt9e013_regs.reg_prev, mt9e013_regs.reg_prev_size);
			// jungki.kim@lge.com  2011-11-22 No more Flicker on Rec for 325 {
			#if 1
			{
				total_lines_per_frame =	mt9e013_regs.reg_prev[E013_FRAME_LENGTH_LINES].wdata;
				mt9e013_group_hold_on();
...
static int mt9e013_sensor_release(void)
{
	...
	kfree(mt9e013_ctrl);
	kfree(lsc_data);	// jungki.kim@lge.com  2011-10-05  Freedom!!!!
	mt9e013_ctrl = NULL;
...
// LGE_DOM_UPDATE_E sungsik.kim 2011/07/23 }
	...
	s->s_mount_angle  = 90; // fix camera sensor orientation by sungsik.kim@lge.com  2011-09-28
